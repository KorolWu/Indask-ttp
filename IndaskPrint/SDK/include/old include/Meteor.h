...
typedef enum {
   ...
   SIG_HDPOWER = 0x00000008,   // Turns the power on or off to heads on a per-PCC basis.  Normally not required, use PiSetHeadPower to control all heads simultaneously.
                               // If hdcnum == 0 and state == 0 all heads on the PCC are powered down.
                               // If hdcnum == 0 and state == 1 all used heads on the PCC are powered up. 
                               // Heads are used if they are listed in the [Planes] .cfg file section.  The used state can also be changed as below.
                               // If hdcnum is set and state == 0 the head is marked as unused and powered down.
                               // If hdcnum is set and state == 1 the head is marked as used.
   ...
   SIG_PDINTERVAL = 0x0000000A,   // Specify the product detect interval.
   ...
   SIG_SPIT = 0x0000000B,   // Do a number of spits (fire on every nozzle) on the specified head.
   ...
   SIG_START_HEAD_READ = 0x00000014,   // Start reading print head memory.  HT_LEXMARK only.
   ...
   SIG_FORCEWHITE = 0x0000001A,   // Force white print data for the head.  Available for most head types (contact support for an up to date list). 
                                  ...
                                  // xaddr BIT0 applies the signal to Head1 Rows 1 and 2; xaddr BIT1 applies the signal to Head1 Rows 3 and 4; xaddr BIT2 applies the signal to Head2 Rows 1 and 2; xaddr BIT3 applies the signal to Head2 Rows 3 and 4
   ...
   SIG_PD_REPEAT = 0x0000002A,   // Specify the product detect repeat count.
   SIGTYPE_COUNT = 0x0000002B,   // Count of valid SigTypes
} SigTypes;
...
typedef enum {
   ...
   HT_LEOPARD = 0x00000001,   // Leopard/Xaar Omnidot318/TTEC CA3/CA4/CE2 etc.
   HT_SPECTRA = 0x00000002,   // Spectra Nova/Galaxy/S-Class
   HT_HSS = 0x00000003,   // Xaar 1001 (HSS)
   HT_KJ4 = 0x00000004,   // Kyocera KJ4
   HT_XJ500 = 0x00000005,   // Xaar XJ 500
   HT_QCLASS = 0x00000006,   // Dimatix Q-Class
   HT_POLARIS = 0x00000007,   // Polaris
   HT_CF1 = 0x00000008,   // TTEC CF1
   HT_SPT_508GS = 0x00000009,   // Seiko 508 greyscale
   HT_RG4 = 0x0000000A,   // Ricoh GEN4
   HT_RG5 = 0x0000000B,   // Ricoh GEN5
   HT_LEXMARK = 0x0000000C,   // Lexmark (Yellowstone)
   HT_SPT_254GS = 0x0000000D,   // Seiko 254 greyscale
   HT_SPT_510BIN = 0x0000000E,   // Seiko 510 binary
   HT_VISTA = 0x0000000F,   // Vista
   HT_STARFIRE = 0x00000010,   // Dimatix Starfire
   HT_SG1024 = 0x00000010,   // Deprecated definition for Dimatix Starfire
   HT_PCCX_TESTBOARD = 0x00000011,   // PCC-X test board
   HT_SPT_RC512 = 0x00000012,   // SII Printek RC512 (requires Meteor version 3.15)
   ...
   HT_Xaar2001 = 0x00000025,   // Xaar2001 head
   ...
   HT_PQ512 = 0x0000002D,   // Dimatix QClass PQ_512 head. New software design for Polaris head (HT_POLARIS)
   HT_QS256 = 0x0000002E,   // Dimatix QClass QS_256 head. New software design for Q-class (Sapphire) head (HT_QCLASS)
   HT_DLase96 = 0x0000002F,   // DataLase laser print module that pretends to be print head
   HT_KM1800I_SH = 0x00000030,   // Konica Minolta KM1800i HDC that drive 1 head
   HT_KY0360_TF = 0x00000031,   // Kyocera KJ4C-0360 head
   HT_KM1024A = 0x00000032,   // Konica Minolta KM1024A head
   HT_RicohGen4 = 0x00000033,   // Ricoh Gen4 head. New software design for RG4 head (HT_RG4)
   HT_RicohGen4L = 0x00000034,   // Ricoh Gen4L head.
   HT_EPSON_S3200 = 0x00000035,   // Epson S3200 head.
   HT_RicohGen5 = 0x00000036,   // Ricoh Gen5/Gen5S head (MH5420,MH5440,MH5220). New software design for RG5 head
   HT_COUNT = 0x00000037,   // Count of HT_xxx values
} eHEADTYPE;
...
#define BMHS_CFGEND 0x80000000   // HDC FPGA config end. Can be used to determine if the head is powered.
#define BMHS_OVERCURRENT 0x40000000   // HDC/Head overcurrent
#define BMHS_BADPIXELVAL 0x20000000   // Top bit of 4bpp pixel data is set
#define BMHS_AMP_TEMP_ALARM 0x04000000   // HDC amplifiers over temperature; head is powered off or waveform set to 0.  Starfire, R5 and KM only
#define BMHS_HIGH 0x02000000   // Always high
#define BMHS_LOW 0x01000000   // Always low
#define BMHS_IO 0x00F00000   // HDC I/O signals
#define BMHS_OVERTEMP 0x00800000   // Head Driver IC overtemp (for KJ4 heads only)
#define BMHS_VAAEN 0x00080000   // Head VaaEnable readback
#define BMHS_VCCEN 0x00040000   // Head VccEnable readback. Can be used to determine if the head is powered.
#define BMHS_CFDNO 0x00020000   // Head CFDNO (Config Done) signal to indicate successful initialisation of waveform on head (if applicable).
#define BMHS_xxxxxxxx 0x00010000   // Spare bit currently
#define BMHS_ADC1 0x0000FF00   // ADC1 value
#define BMHS_ADC0 0x000000FF   // ADC0 value
#define BMHS_HEAD_REQUIRED 0x01000000   // Head required by config file
#define BMHS_L_ADC 0x0000FFFF   // ADC result. The 10-bit value is actually in bits 11 - 2 of this field due to a quirk of the AD7477 ADC
#define BMHS_H_LVDSLOCK 0x08000000   // Lvds Lock signal
#define BMHS_H_SUPPLYOK 0x04000000   // HV supply OK
...
#define DATAPATH_FEATURE_CFIMAGE 4   // PCMD_CFIMAGE available: see <see cref="F:Ttp.Meteor.TAppStatus.bmDatapathsAvailable" />
...
#define BM_PD_ACTIVE_LOW 0x00000004   // Product-detect is active-low
#define BM_QUAD 0x00000008   // Shaft-encoder is quadrature
#define BM_INT_SE 0x00000010   // Use 'internal' shaft-encoder
#define BM_spare1 0x00000020   // Spare
#define BM_spare2 0x00000040   // Spare
#define BM_DIRINV 0x00000080   // Invert encoder input direction
...
#define BM_REVDIR 0x00008000   // Set to reverse transport (encoder) direction
...
typedef enum {
   ...
   JT_SCAN_DIRECT = 0x00000005,   // JT_SCAN_DIRECT is JT_SCAN (fifo path enabled) with print data sent via PCMD_UCMD.  Deprecated and should not be used.
...
typedef enum {
   ...
   PCMD_STARTJOB = 0x7A5535A1,   // StartJob command id
                                 ...
                                 // Cmd[1] = 4 (DWORD count)
                                 // Cmd[2] = Job ID
                                 // Cmd[3] = Job Type <see cref="T:Ttp.Meteor.eJOBTYPE">eJOBTYPE</see>and <see cref="F:Ttp.Meteor.MeteorConsts.JT_FLAG_CONTINUE" />
   ...
   PCMD_IMAGE = 0x7A5535A4,   // SendImage command id
                              ...
                              // Cmd[2] = [Bits7:0] Plane (1-<see cref="F:Ttp.Meteor.MeteorConsts.MAX_PLANES">MAX_PLANES</see>)
                              // Cmd[3] = XLeft (pixels) or XStart (pixels) for scanning or continuous print; XRight (pixels) if JT_FLAG_RIGHTALIGN is set
                              // Cmd[4] = YTop (pixels)
                              // Cmd[5] = Width (pixels)
                              // Cmd[6-N] = Image data
   PCMD_HDIMAGE = 0x7A5535A5,   // SendHeadImage command id
                                // Cmd[0] = PCMD_HDIMAGE
                                // Cmd[1] = Command DWORD count (3 + head image DWORDs)
                                // Cmd[2] = PCC number (1-pcc count)
                                // Cmd[3] = Head number (1-8)
                                // Cmd[4] = XStart (pixels)
   ...
   PCMD_UCMD = 0x7A5535AD,   // UCMD (passthru) command id
   PCMD_CFIMAGE = 0x7A5535AE,   // CompactFlashImage command id
                                // Cmd[0]  = PCMD_CFIMAGE
                                // Cmd[1]  = 12 (DWORD count)
                                // Cmd[2]  = Colour plane (1-MAX_PLANES)
                                // Cmd[3]  = X left co-ordinate [bits 0:27]; CFMASK_xxx [bits28:31]
                                // Cmd[4]  = Y top
                                // Cmd[5]  = Image index in CF card (1-N)
                                // Cmd[6]  = Top left X
                                // Cmd[7]  = Top left Y
                                // Cmd[8]  = Top right X
                                // Cmd[9]  = Top right Y
                                // Cmd[10] = Bottom left X
                                // Cmd[11] = Bottom left Y
                                // Cmd[12] = Bottom right X
                                // Cmd[13] = Bottom right Y
   PCMD_CF_WRITEFILE = 0x7A5535AF,   // CompactFlash WriteFile command id
   PCMD_TILE = 0x7A5535B0,   // Tile command id (USB hardware only)
   PCMD_CF_WRITE = 0x7A5535B2,   // CompactFlash Write command id
                                 ...
                                 // Cmd[4-N] = Image data
   PCMD_CF_HDIMAGE = 0x7A5535B3,   // CompactFlash HeadImage command id
                                   // Cmd[0] = PCMD_CF_HDIMAGE
                                   // Cmd[1] = 5 or 6 (Command DWORD count)
                                   // Cmd[2] = PccNum (1 - N)
                                   // Cmd[3] = HNum (1 - 8)
                                   // Cmd[4] = X Start (pixels)
                                   // Cmd[5] = X Length (pixels)
                                   // Cmd[6] = Starting CF DWORD address.
                                   // Cmd[7] = [Optional] Top bits of CF DWORD address for cards larger than 16GB
   ...
   PCMD_CF_HDIMAGE_BLK = 0x7A5535B7,   // CompactFlash block aligned HeadImage command id
                                       ...
                                       // Cmd[2] = PccNum (1 - N)
                                       // Cmd[3] = HNum (1 - 8)
                                       // Cmd[4] = BITS0-24: X Start (pixels); see above for flags in upper 4 bits
                                       // Cmd[5] = X Length (pixels)
                                       // Cmd[6] = Starting CF block address
   PCMD_PRETRANSLATED = 0x7A5535B8,   // Send offline translated data. This command is deprecated and no longer supported.
                                      // Cmd[0] = PCMD_PRETRANSLATED
                                      // Cmd[1] = Filename DWORD count (including nullchar)
                                      // Cmd[2-N] = Filename of offline translated file, as ascii string with terminating nullchar.
   ...
   PCMD_PRINT_RAM_IMAGE = 0x7A5535BA,   // Print Image Stored in PCC RAM
                                        ...
                                        // Cmd[1] = Command DWORD count: 3 for standard printing, 4 for circular printing
                                        ...
                                        // Cmd[5] = [Optional] X Offset (pixels) within the image for circular printing
   ...
   PCMD_WRITE_RAM_IMAGE_EX = 0x7A5535BE,   // Write Image to PCC RAM
                                           ...
                                           // the "more images" flag (BIT30 of Cmd[4]) must be set for all but the final image in the sequence.
   ...
   PCMD_CF_TRANSLATEIMAGE = 0x7A5535BF,   // Prepare an image for writing to Flash memory (PCC8 Compact Flash or PCCE UHS SD Card) using the translator
                                          ...
                                          // Cmd[3] = YTop (pixels)
                                          // Cmd[4] = Width (pixels).  If an image buffer is used, and the buffer contents were loaded from a file 
                                          // (PiAllocateTiffImageBuffer etc.), then Width should be set to zero (which means use the image buffer's width).
                                          ...
                                          // Cmd[8-15] = [Optional] Reserved; should be zero
   ...
   PCMD_RIPIMAGE = 0x7A5535C5,   // Rip-and-print image command id
                                 ...
                                 // default RIP config
                                 // For managed code, the PiSendRipImageCommandHelper method can be used to hide the marshalling of the two strings
                                 // For unmanaged code, the strings must be wchar_t pointers
   ...
   PCMD_ROTATE = 0x7A5535C6,   // Rotate an image in range of [-45, +45] degrees immediately before translating.
                               ...
                               // Cmd[0] = PCMD_ROTATE
                               // Cmd[1] = 4 (command DWORD count)
                               // Cmd[2] = Signed 32-bit integer describing rotation angle in 10^-7 degree steps, i.e. rotation angle = Cmd[2]/10^7.
                               // Cmd[3] = Reserved for future use.  Must be zero.
                               // Cmd[4] = Reserved for future use.  Must be zero.
                               // Cmd[5] = Reserved for future use.  Must be zero.
                               // Cmd[6] = Reserved for future use.  Must be zero.
...
typedef enum {
   ...
   CCP_SCLK = 0x00000029,   // SCLK for HSS head (period in ns (85 - 1000))
   ...
   CCP_XRES_SCALE = 0x00000037,   // Scale X resolution with multiplier (ms 16 bits) and divider (ls 16 bits)
   ...
   CCP_V_ADJUST_EX = 0x0000003C,   // HT_RG4, HT_RG5: Vp in 0.1v steps; 0 to use waveform's original voltage.
                                   ...
                                   // HT_QCLASS, HT_POLARIS: anum = 1 for odd piezo waveform Voltage scaling for HDC-Q Jetting Assembly 1 (0.1%).  Combined with "HeadVoltageAdjustment" config file setting if present.
                                   // HT_QCLASS, HT_POLARIS: anum = 2 for even piezo waveform Voltage scaling for HDC-Q JA1 (0.1%).  Combined with "HeadVoltageAdjustment" config file setting if present.
                                   // HT_QCLASS, HT_POLARIS: anum = 3 for odd piezo waveform Voltage scaling for HDC-Q JA2 (0.1%).  Combined with "HeadVoltageAdjustment" config file setting if present.
                                   // HT_QCLASS, HT_POLARIS: anum = 4 for even piezo waveform Voltage scaling for HDC-Q JA2 (0.1%).  Combined with "HeadVoltageAdjustment" config file setting if present.
   ...
   CCP_SCANNING = 0x0000003F,   // Switch Meteor between scanning mode (non-zero) and fixed head mode (zero).
   ...
   CCP_KHEAD_TEMP_LIMITS = 0x00000044,   // Deprecated.  Use CPEX_TemperatureUpperLimit and CPEX_TemperatureLowerLimit.
   CCP_K300_ABS_VOLTAGE = 0x00000045,   // Deprecated.  Use CPEX_HeadVoltageAbs.
   CCP_K600_ABS_VOLTAGE = 0x00000046,   // Deprecated.  Use CPEX_HeadVoltageAbs.
...
typedef enum {
   ...
   CPEX_TargetTemp = 0x00000116,   // Target temperature
                                   ...
                                   // HT_SG600, HT_KY1200RM, HT_QS256, HT_PQ512, HT_RicohGen4, HT_RicohGen4L
   ...
   CPEX_TickleWF_FileIdx = 0x00000120,   // Tickle waveform file index. Note: indexing starts from 1
                                         // Head Types: HT_SAMBA, HT_GH2220, HT_RG4 (legacy), HT_RG5, HT_RicohGen4, HT_RicohGen4L, HT_KM1800I, HT_KM1800I_SH, HT_KM1024I,
   ...
   CPEX_WF_VScaleMode = 0x00000121,   // Waveform voltage scale mode 0,1,2
                                      // Head Types: HT_SAMBA, HT_GMA33, HT_GMA99, HT_KM_M600, HT_Xaar1201, HT_SG600
                                      // Zero means voltage scaling disabled
                                      // One means keep hold time constant (HT_KM_M600, HT_Xaar1201) or keep pulse width constant (Dimatix heads)
   ...
   CPEX_WF_VScaleCoeff = 0x00000122,   // Waveform voltage scale coefficient (0.5 to 1.5)
                                       // Head Types: HT_SAMBA, HT_GMA33, HT_GMA99, HT_SG600, HT_KM130, HT_KM_M600, HT_KM1024I (scales VH2 only), HT_KM1800I (scales VH2 only), 
                                       // HT_KM1800I_SH (scales VH2 only), HT_KM1024 (scales VH2 only), HT_KM1024A (scales VH2 only), HT_Xaar1201, HT_RicohGen4, HT_RicohGen4L, 
                                       // HT_GH2220, HT_RG5, HT_RG4 (legacy), HT_STARFIRE, HT_PQ512, HT_QS256
   ...
   CPEX_HeadSubType = 0x00000148,   // Head subtype. Some heads can have slightly different variations, like XAAR2001 GS12/GS40U
                                    // Head Types: HT_Xaar2001 (0 for GS6/GS12; 1 for GS40)
...
typedef struct {
   ...
   eHEADTYPE   HeadType;   // 支持的喷头类型<see cref="T:Ttp.Meteor.eHEADTYPE">Head type</see>
   int32   Control;   // <see cref="T:Ttp.Meteor.Bmcontrol">Control word</see>
   int32   NumPlanes;   // Number of defined colour planes
   int32   Yinterlace;   // 交错的头数Number of heads interlaced
   int32   PccsRequired;   //  配置文件里面配的PCC数量 Pccs required by config file
   int32   PccsAttached;   // 实际连接的PCC数量 Number of PCCs currently attached
   ePRINTERSTATE   PrinterState;   // <see cref="T:Ttp.Meteor.ePRINTERSTATE">Overall printer state</see>
   int32   PrintSpeed;   // 打印速度（打印行/秒）Print speed (print lines/second)
   int32   DocIds[64];   // 自上次状态调用以来在“通知”标题处看到的文档ID列表List of document IDs seen at 'notify' head since last status call
   int32   DocCount;   // Doc count since last status call (i.e. number of Ids in the DocIds array)
   int32   PdCount;   // 产品检测计数 Product-detect count
   int32   PrintCount;   //当前作业中的打印记数 Print-count in current job
   int32   Xdpi;   // Calculated X-resolution
   int32   Ydpi;   // Calculated Y-resolution
   ...
   int32   bmMeteorStatus;   // 附加的状态位 Additional meteor status bits BMMS_xxx <see cref="T:Ttp.Meteor.Bmms" />
   int32   PeBuildNumber;   // 打印引擎的版本号 Print Engine build number
   int32   SupportedBppBitmask;   // 例如0x2表示1bpp（位1）。0x4表示2bpp（位2）。0x10表示4bpp（位4）BitN (0-31) set if Meteor can accept N bits-per-pixel data for the current head type e.g. 0x2 means 1bpp (bit1). 0x4 means 2bpp (bit2). 0x10 means 4bpp (bit4)
   int32   BitsPerPixel;   // 当前选择的位深度 Currently selected bits per pixel
   int32   PrintInterval;   // 内部产品检测打印间隔的当前值，如果使用外部产品检测，则为零 Current value of the internal product detect print interval, or zero if external product detect is in use
   ...
   int32   JobCountLane2;   // Total number of print jobs (including the active job, if any) started by the current instance of the PrintEngine on print lane 2.
                            // Can read zero while hardware is initialising.  Ethernet hardware only.
   int32   InitCount;   // PrintEngine的当前实例已（重新）初始化的次数 Number of times the current instance of the PrintEngine has been (re-)initialised
} TAppStatus;

// Pcc Status Structure
typedef struct {
   int32   StructVersion;   // <see cref="F:Ttp.Meteor.MeteorConsts.APP_PCC_STATUS_VERSION">Structure version</see>
   int32   IoSignals;   // I/O signal states for PCC and 6 HDCs
   int32   bmStatusBits;   // Status word from PCC, see <see cref="T:Ttp.Meteor.Bmps">Bmps</see>
   ...
   int32   PdCount2;   // Aux product-detect count (PCCE only)
} TAppPccStatus;
...
typedef enum {
   ...
   HUS_UNINITIALISED = 0x00000001,   // Head powered off, FPGA unitialised etc
...
typedef struct {
   ...
   int32   bmStatusBits;   // Status word from HDC
   int32   PreloadDataUsed;   // DDRAM % used on preload path
   int32   FifoDataUsed;   // DDRAM % used on FIFO path
   int32   Temperature1;   // HDC temperature 1 (0.1C):
                           // Head temp HT_LEOPARD, HT_HSS, HT_XJ500, HT_K150, HT_K300, HT_K600, HT_K1200, HT_KJ4, HT_SPT_254GS, HT_SPT_510BIN, HT_LEXMARK, HT_SPT_508GS, HT_STARFIRE, HT_GMA33, HT_GMA99, HT_SAMBA, HT_RG5, HT_SG600
                           // Head 1 temp for HT_KM1800I, HT_KM1024I, HT_KM1024;
                           // Head temp A for HT_CF1, HT_SPECTRA, HT_QCLASS, HT_Xaar2001, HT_RG4;
                           ...
                           // Head amp temp A for HT_Xaar1201, HT_KY0360;
   int32   Temperature2;   // HDC temperature 2 (0.1C):
                           // Head2 temp for HT_KM1800I, HT_KM1024I, HT_KM1024;
                           // Head temp B for HT_CF1, HT_SPECTRA, HT_QCLASS, HT_Xaar2001, HT_RG4;
                           ...
                           // Head amp temp B for HT_Xaar1201, HT_KY0360;
   ...
   int32   bmStatusBits2;   // Second status word from HDC
   int32   Temperature3;   // HDC temperature 3 (0.1C):
                           ...
                           // Head temp C for HT_RG4;
                           // Head amp temp A for HT_STARFIRE;
                           // Head amp temp B for HT_SAMBA, HT_SPT_508GS; HT_SG600
                           // Head amp temp C for HT_Xaar1201;
                           // Head aux temp A for HT_QCLASS;
   ...
   int32   bmStatusBits3;   // Additional status bits
...
typedef struct {
   ...
   int32   WritesQueued;   // Count of queued write commands
   ...
   int32   DirectDdramAccess;   // Non-zero if the PCMD_CF_xxx commands are being routed to preoad RAM rather than the UHS SD card (PCCE only)
...
// generic subsystem (e.g. PCC/HDC/HEAD) data represented in generic format (JSON)
typedef struct {
   char   JsonText[4096];   // JSON text string
} TGenericSubSystemData;

// Structure records the raw EEPROM data from the QClass printhead.  There can be two heads (JA0 and JA1) connected to each HDC-Q.
typedef struct {
   int32   StructVersion;   // Structure Version
   int32   PccNumber;   // Pcc number (1 - n) for this record
   int32   HeadNumber;   // Head number (1 - n) for this record
   uint8   Revision0[3];   // Revision
   uint8   PartNumber0[10];   // Part Number
   uint8   SerialNumber0[12];   // Serial Number
   uint8   CalibrationVoltage0[4];   // Calibration Voltage
   uint8   DropMassOffset0[6];   // Drop Mass Offset
   uint8   Pitch0[7];   // Pitch
   int32   DataValid0;   // Data Valid (JA0).  Non-zero if the JA0 data checksum has verified successfully.  If zero the JA0 data above should be ignored.
   uint8   Revision1[3];   // Revision
   uint8   PartNumber1[10];   // Part Number
   uint8   SerialNumber1[12];   // Serial Number
   uint8   CalibrationVoltage1[4];   // Calibration Voltage
   uint8   DropMassOffset1[6];   // Drop Mass Offset
   uint8   Pitch1[7];   // Pitch
   int32   DataValid1;   // Data Valid (JA1).  Non-zero if the JA1 data checksum has verified successfully.  If zero the JA1 data above should be ignored.
} TAppQClassEeprom;

// Structure to store data read back from the security chip fitted to some HDCs
// This structure is now defined here only
typedef struct {
   int32   DataReady;   // Flag set if the security chip data has been read.  If FALSE the Meteor Print Engine is
                        ...
                        // later.
   int32   StatusBits;   // Head Status bits
   uint8   SerialNumber[9];   // head serial number
} SerialNumberStatus;
...
typedef struct {
   ...
   int32   HssAsicCentiVolts[16];   // Voltage (x100) applied to HSS ASIC
   int32   HssAsicTrimCentiVolts[16];   // PrintEngine component of HSS ASIC trim voltage (x100)
...
typedef struct {
   ...
   int32   BitsPerPixel[32];   // Bit depth for print data sent to the plane.  Noramally the same for each plane.  
...
// For future use.  Determines how pixels are decoded from a multi-channel file
// into a Meteor image buffer.
typedef enum {
...
// Holds details of how multi-channel print data is arranged in a file.  It is the application's
// responsibility to decode the details of the file header and fill in the structure details.
// This is suitable for file which:
...
typedef struct {
   ...
   FilePixelDecodeMode   DecodeMode;   // Determines how the pixels are arranged in each 32 bit word
...
typedef enum {
   ...
   IMG_LOAD_ROTATE = 0x00000400,   // Should the image be rotated according to the structure's rotation values
} bmImageLoadFlags;
...
typedef struct {
   ...
   int32   OneBppMapMode;   // [In] Gray level to use when expanding a one bit per pixel file to two or four bits per pixel
   int32   BufferWidth;   // [In] Buffer width if the buffer is created larger than the (cropped) image - 
                          // only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE or IMG_LOAD_X_REPEAT is set in Flags
   int32   BufferHeight;   // [In] Buffer height if the buffer is created larger than the (cropped) image - 
                           // only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE is set in Flags
   int32   BufferXPos;   // [In] X position within the buffer - only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE is set in Flags
   int32   BufferYPos;   // [In] Y position within the buffer - only used if IMG_LOAD_EXTEND or IMG_LOAD_COMBINE is set in Flags
   ...
   int32   RotateDegrees;   // [In] The degrees that the image will be rotated by. 
...
typedef struct {
   ...
   int32   FirstPage;   // First page of a multi-page PDF to RIP; 0 for all pages or if the file is not a multi-page document
   int32   LastPage;   // Last page of a multi-page PDF to RIP; 0 for all pages or if the file is not a multi-page document
   int32   ControlFlags;   // BIT0 if further calls for the same file are expected; BIT1 to get the next page from a previously started RIP; 
                           // BIT2 to abort a running RIP; BIT3 for double sided mode
   int32   FirstAltPlane;   // Index (zero based) of the first plane for the alternate pages in double sided mode
} RipDetails;
...
typedef struct {
   int32   StructureSizeBytes;   // [In] Size of the AttachSpitBarDetails structure in bytes.  Must be filled in by the user.  
                                 // Used to protect against version inconsistencies.
   int32   MemBlockXLen;   // [In] Memory block size (for locating the alternative ending in the next block)
   int32   MemBlockFinalXLen;   // [In] Size of the final memory block, which may be 0.5 to 1.5 times the size of the standard mem block.
   int32   OriginalImageWidth;   // [In] Width of the image before we increased it to make room for the alternative ending.
   int32   AddedAltEndingWidth;   // [In] Width of the image added by the alternative ending.
   int32   AddedSpitBarWidth;   // [In] Width of the image added by the spit-bar.
   int32   SpitBarOffset;   // [In] The distance in pixels between the end of the image and the spit-bar
   int32   SpitBarWidth;   // [In] The width of the spit-bar in pixels
   int32   PaddingAndEndBlocks;   // [In] The number of blocks to put before the alternative spit bar, to account for nozzle row separation in translation
                                  // and extra blocks required to be in the end section due to long spit-bar separations between planes.
   bool   SpitBarRight;   // [In] Which side to put the spit-bar on, left or right.
...
typedef struct {
   double   PeakTransferMbPerSecond;   // Peak transfer rate in the sample period in MB/s
   uint32   IdlePercent;   // Percentage of the sample period where the thread was idle
   uint32   DwordsSent;   // Number of processed during the sample period
   uint32   DwordsInQueue;   // The number of dwords in the queue at the end of the sample period
   uint32   TcpMss;   
   uint32   TcpCwnd;   
   uint32   TcpSndWnd;   
   uint32   TcpRcvWnd;   
   uint32   TcpBytesRetrans;   
   uint32   TcpFastRetrans;   
   uint32   TcpTimeoutEpisodes;   
   uint32   TtcpSynRetrans;   
} TPerfMonRecord;

typedef struct {
   TPerfMonRecord   PccStats[255];   // Individual measurements for every PCC in the system
   TPerfMonRecord   CombinedPccStats;   // Averages / accumulations of the measurements for every PCC
   int32   PccCount;   // The number of PCCs which we are collecting statistics for
} TSystemPerfMonRecord;

#define KMaxPerfMonRecords 0x00000010   
#define KPerfMonShareName "MonitorPerfMon"   

typedef struct {
   int32   StructureSizeBytes;   
   TSystemPerfMonRecord   PerfMonQueue[16];   
   int32   InIndex;   
   int32   OutIndex;   
} TPerfMonShare;
...
typedef enum {
   ...
   STATUS_QCLASS_EEPROM = 0x00000004,   // Request TAppQClassEeprom
   STATUS_DROP_TUNER = 0x00000005,   // Request DropTunerStatus
   STATUS_CONTINUOUS_PRINT = 0x00000006,   // Request TContinuousPrintStatus
   STATUS_VISTA = 0x00000007,   // Request TVistaStatus
   STATUS_LEXMARK = 0x00000008,   // Request LexmarkStatus
   STATUS_SG1024_EEPROM = 0x00000009,   // Request TAppSG1024Eeprom
   STATUS_RG4HDC_EEPROM = 0x0000000A,   // Request TAppRG4HDCEeprom
   STATUS_RG5_EEPROM = 0x0000000B,   // Request TAppRG5Eeprom
   STATUS_SWATHSEPARATOR = 0x0000000C,   // Request SwathSeparatorStatus
   STATUS_SWATHDETAILS = 0x0000000D,   // Request SwathDetails
   STATUS_EEPROM_DATA = 0x0000000E,   // Generic request for head EEPROM contents in JSON format
   STATUS_HDC_SERIAL_NUM = 0x0000000F,   // Request HDC ATSHA serial number
   STATUS_PCC_SERIAL_NUM = 0x00000010,   // Request PCC ATSHA serial number
   STATUS_PLANE_CONFIG = 0x00000011,   // Request for plane config status
   STATUS_CONFIG_DESCRIPTOR = 0x00000012,   // Request for config. descriptor status
   STATUS_GEOMETRYSUMMARY = 0x00000013,   // Request geometry summary
   STATUS_ENUM_COUNT = 0x00000014,   // must be the last member of this enum
} eSTATUSTYPE;
